// Listing directories https://www.nextflow.io/docs/latest/working-with-files.html#listing-directories
...
// where to store separate scripts
...
params.use_probabilities = "false"
...
process protein_query_qc {

input:
 file query

output:
 file "query_single.fasta"
 file "input_seq_char_counts.log" optional true
...
output:
 ...
 file "report.blast"
 file "blast_output_*_filtered.tsv"
 file "headers_mapping.txt" optional true
 file "encoded_headers.txt"
 file "*.taxids"
...
if [ `cat relatives.taxids | wc -l` -eq 0 ]; then
	...
	echo "Maybe this species is single in the family, so pipeline can build incorrect tree" >&2
...
workflow {
	...
	if (params.verbose) {
		...
		iqtree anc model : ${params.iqtree_anc_model}
		outdir       : ${params.outdir}

		Collect mutational spectra of mutations:
			all: ${params.all}"
			syn: true"
			syn4f: ${params.syn4f}"
			non-syn: ${params.nonsyn}"
		
		Minimal number of mutations to save 192-component spectrum (mnum192): ${params.mnum192}"
		Minimal number of sequences (leaves in a tree ingroup) to run the pipeline: ${params.required_nseqs}"
		Use probabilities: ${params.use_probabilities}"
		Mutation probability cutoff: ${params.proba_cutoff}"
		Run simulation: ${params.run_simulation}"
		Threads: ${THREADS}"
		Run treeshrink: ${params.run_shrinking}"
		Shrinking Quantile: ${params.quantile}"
		Exclude conservative sites: ${params.exclude_cons_sites}"
		internal branches spectra: ${params.internal}"
		terminal branches spectra: ${params.terminal}"
		branch-scpecific spectra: ${params.branch_spectra}"
		Save expected mutations: ${params.save_exp_mutations}"
		Use uncertainty coefficient: ${params.uncertainty_coef}"
    ...
    params.gencode
    params.species_name
    ch_protein_sequences = Channel.fromPath(params.sequence)
    
    protein_query_qc(query_protein_sequence)
